...
def join_pool(db: Session, pool: Pool, participant_data: PoolParticipantBase) -> PoolParticipant:
    ...
    try:
        ...
        if (
            "uq_pool_wallet" in error_text
            or "pool_participants.pool_id, pool_participants.wallet_address" in error_text
        ):
...
def get_pool_participants(db: Session, pool_id: int) -> list[PoolParticipant]:
    return (
        db.query(PoolParticipant)
        .filter(PoolParticipant.pool_id == pool_id)
        .order_by(
            PoolParticipant.current_streak.desc(),
            PoolParticipant.days_completed.desc(),
            PoolParticipant.joined_at.asc(),
        )
        .all()
    )
...
def submit_checkin(db: Session, pool: Pool, checkin_data: PoolCheckinCreate) -> PoolCheckin:
    ...
    try:
        ...
        error_text = str(error.orig).lower()
        if (
            "uq_participant_checkin" in error_text
            or "pool_checkins.participant_id, pool_checkins.check_in_date" in error_text
        ):
    ...
    checkin_dates = [
        item.check_in_date
        for item in get_checkins_for_wallet(db, pool_id=pool.id, wallet_address=participant.wallet_address)
    ]
